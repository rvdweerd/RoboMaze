...
public:
	...
	{
		const unsigned int alpha = c.GetA();
		const Color pre_src = Color(
			(c.GetR() * alpha) / 256,
			(c.GetG() * alpha) / 256,
			(c.GetB() * alpha) / 256
		);
		const unsigned int cAlpha = 255u - alpha;

		for( int y = y0; y < y1; ++y )
		{
			for( int x = x0; x < x1; ++x )
			{
				const Color dst = GetPixel( x,y );
				const int rb_dest = (((dst.dword & 0xFF00FFu) * cAlpha) >> 8) & 0xFF00FFu;
				const int g_dest = (((dst.dword & 0x00FF00u) * cAlpha) >> 8) & 0x00FF00u;
				PutPixel( x,y,pre_src.dword + rb_dest + g_dest );
			}
		}
	}
	void DrawRect( const RectI& rect,Color c,const RectI& clip )
	{
		const auto clipped = rect.GetClippedTo( clip );
		DrawRect( clipped.left,clipped.top,clipped.right,clipped.bottom,c );
	}
	template<typename E>
