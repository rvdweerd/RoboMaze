...
MainWindow::MainWindow( HINSTANCE hInst,wchar_t * pArgs,const Config& config )
	:
	args( pArgs ),
	hInst( hInst )
{
	...
	RECT wr;
	wr.left = 350;
	wr.right = config.GetScreenWidth() + wr.left;
	wr.top = 100;
	wr.bottom = config.GetScreenHeight() + wr.top;
...
LRESULT MainWindow::HandleMsg( HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam )
{
	...
	{
	case WM_DESTROY:
		if( exitMode == ExitMode::Invalid )
		{
			exitMode = ExitMode::Kill;
		}
		PostQuitMessage( 0 );
		break;
	case WM_KILLFOCUS:
